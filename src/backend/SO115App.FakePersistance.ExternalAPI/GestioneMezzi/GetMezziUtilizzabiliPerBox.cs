...
namespace SO115App.ExternalAPI.Fake.GestioneMezzi
{
    public class GetMezziUtilizzabiliPerBox : IGetMezziUtilizzabiliPerBox
    {
        ...
        public async Task<Dictionary<string, string>> Get(string[] CodiciSedi)
        {
            ...
            new KeyValuePair<string, string>(mezzo.CodiceMezzo, lstStatiMezzi.Result.LastOrDefault(stato => stato.CodiceMezzo.Equals(mezzo.CodiceMezzo) && !stato.StatoOperativo.Equals("Rientrato"))?.StatoOperativo ?? Costanti.MezzoInSede))
                .Distinct()
