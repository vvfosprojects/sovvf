    ...
    public class PersonaleService
    {
        ...
        public async Task<List<SquadreNelTurno>> GetSquadreNelTurno(string codiceSede, string codiceTurno)
        {
            try
            {
                ...
                var response = await client.GetStringAsync(string.Format(Costanti.ServiziGetSquadreUrl + "/GetSquadreNelTurno/codiceSede={0}&codiceTurno={1}", codiceSede, codiceTurno));
                ...
                foreach (var turno in ListTurno)
                {
                    foreach (var squadra in turno.ListaSquadre)
                    {
                        ...
                        var responseComponenti = await client.GetStringAsync(string.Format(Costanti.ServiziGetSquadreUrl + "/codiceSede={0}&codiceSquadra={1}&codiceTurno={2}", codiceSede, squadra.Codice, turno.Codice));
        ...
        public async Task<List<SquadreNelTurno>> GetSquadreBySede(string codiceSede)
        {
            ...
            {
                List<SquadreNelTurno> ListaSquadreTurno = new List<SquadreNelTurno>();
                var response = await client.GetStringAsync(string.Format(Costanti.ServiziGetSquadreUrl + "/GetSquadreBySede?codiceSede={0}", codiceSede));
                ...
                foreach (var turno in ListTurno)
                {
                    foreach (var squadra in turno.ListaSquadre)
                    {
                        List<Componente> ListaComponenti = new List<Componente>();
                        squadra.ListaComponenti = new List<Componente>();
                        var responseComponenti = await client.GetStringAsync(string.Format(Costanti.ServiziGetComponentiUrl + "?codiceSede={0}&codiceSquadra={1}&codiceTurno={2}", codiceSede, squadra.Codice, turno.Codice));
                        ListaComponenti = JsonConvert.DeserializeObject<List<Componente>>(responseComponenti);
                        squadra.ListaComponenti = ListaComponenti;
