...
namespace SO115App.ExternalAPI.Fake.Servizi.GeoFleet
{
    ...
    public class GetPosizioneByCodiceMezzo : IGetPosizioneByCodiceMezzo
    {
        ...
        public async Task<MessaggioPosizione> Get(string codiceMezzo)
        {
            ...
            var response = await _client.GetAsync(_configuration.GetSection("UrlExternalApi").GetSection("GeofleetApi").Value + Costanti.GeoFleetGetPosizioneByCodiceMezzo + "posizioneByCodiceMezzo?id=" + codiceMezzo).ConfigureAwait(false);//L'API GeoFleet ancora non si aspetta una lista di codici mezzo
