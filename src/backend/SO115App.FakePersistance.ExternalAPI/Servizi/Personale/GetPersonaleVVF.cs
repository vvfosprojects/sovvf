...
{
    /// <summary>
    ///   classe che implementa il servizio per il recupero del personale da Utenti Comuni
    /// </summary>
    public class GetPersonaleVVF : IGetPersonaleVVF
    {
        ...
        public async Task<List<PersonaleVVF>> Get(string text, string codSede = null)
        {
            ...
            var response = await _client.GetAsync($"{_configuration.GetSection("UrlExternalApi").GetSection("UrlAPIGestioneUtente").Value}?searchKey={text}").ConfigureAwait(false);
