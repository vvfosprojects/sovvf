    ...
    public class GetUtenti : IGetUtenti
    {
        ...
        public List<Utente> Get(string codiceSede, string cercaNoC = null)
        {
            return cercaNoC == null
                ? _dbContext.UtenteCollection.Find(Builders<Utente>.Filter.Eq(x => x.Sede, codiceSede)).ToList()
                : _dbContext.UtenteCollection.Aggregate().Match(m => m.Nome.Contains(cercaNoC) || m.Cognome.Contains(cercaNoC)).ToList().FindAll(x => x.Sede.Equals(codiceSede));
        }
    }
}
