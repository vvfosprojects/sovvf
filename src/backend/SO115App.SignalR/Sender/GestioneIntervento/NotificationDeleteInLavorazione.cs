...
namespace SO115App.SignalR.Sender.GestioneIntervento
{
    public class NotificationDeleteInLavorazione : INotifyDeleteInLavorazioneRichiesta
    {
        ...
        public async Task SendNotification(RimozioneInLavorazioneCommand intervento)
        {
            ...
            await _notificationHubContext.Clients.Group(intervento.Chiamata.Operatore.Sede).SendAsync("NotifyGetBoxInterventi", boxInterventi);
            await _notificationHubContext.Clients.Group(intervento.Chiamata.Operatore.Sede).SendAsync("NotifyGetRichiestaUpDateMarker", listaSintesiMarker.LastOrDefault(marker => marker.Codice == intervento.Chiamata.Codice));
        }
    }
}
