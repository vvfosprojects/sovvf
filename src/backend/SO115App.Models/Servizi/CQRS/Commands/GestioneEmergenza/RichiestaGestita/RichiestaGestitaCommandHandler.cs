...
{
    public class RichiestaGestitaCommandHandler : ICommandHandler<RichiestaGestitaCommand>
    {
        private readonly IUpDateEmergenza _upDateEmergenza;
        private readonly IGetEmergenzaById _getEmergenzaById;

        public RichiestaGestitaCommandHandler(IUpDateEmergenza upDateEmergenza, IGetEmergenzaById getEmergenzaById)
        {
            _upDateEmergenza = upDateEmergenza;
            _getEmergenzaById = getEmergenzaById;
        }

        public void Handle(RichiestaGestitaCommand command)
        {
            var InfoEmergenza = _getEmergenzaById.Get(command.Id);

            foreach (var evento in InfoEmergenza.ListaEventi)
            {
                if (evento is RichiestaEmergenza)
                {
                    if (evento.Istante.Equals(command.richiestaEmergenza.Istante))
                    {
                        ((RichiestaEmergenza)evento).Gestita = true;
                        ((RichiestaEmergenza)evento).istanteGestione = DateTime.Now;
                    }
                }
            }
