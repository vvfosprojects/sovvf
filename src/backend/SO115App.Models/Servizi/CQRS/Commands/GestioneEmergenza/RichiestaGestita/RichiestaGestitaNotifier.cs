...
{
    public class RichiestaGestitaNotifier : ICommandNotifier<RichiestaGestitaCommand>
    {
        private readonly ICallMatrix _callMatrix;
        private readonly INotifyRichiestaEmergenza _sender;

        public RichiestaGestitaNotifier(ICallMatrix callMatrix, INotifyRichiestaEmergenza sender)
        {
            _callMatrix = callMatrix;
            _sender = sender;
        }

        public void Notify(RichiestaGestitaCommand command)
        {
            _sender.Send(command.InfoEmergenza);

            var messaggio = $"La richesta di intervento per l'emergenza codice {command.InfoEmergenza.CodEmergenza}. Coordinate: x:{command.InfoEmergenza.Localita.Coordinate.Longitudine} y:{command.InfoEmergenza.Localita.Coordinate.Latitudine} è stata gestita";
