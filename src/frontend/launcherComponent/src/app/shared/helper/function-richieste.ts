...
export function getPartenzeAttive(partenze: Partenza[], codMezziRientratiVisibili?: MezzoRientratoVisibileRichiesta, annullaStatoMezzoRientrato?: InfoMezzo): Partenza[] {
  const partenzeNew = getUniqueLastPartenze(partenze);
  return partenzeNew.filter((p: Partenza) => (!p.partenza.sganciata && !p.partenza.partenzaAnnullata && p.partenza.mezzo.stato != StatoMezzo.Rientrato) || codMezziRientratiVisibili?.codMezzi?.includes(p.partenza.mezzo.codice) || annullaStatoMezzoRientrato?.codMezzo === p.partenza.mezzo.codice);
}
