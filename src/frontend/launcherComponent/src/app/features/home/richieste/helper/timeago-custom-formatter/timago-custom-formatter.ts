...
const defaultFormattter = function (then: number): { value: number, unit: CustomUnit, suffix: CustomSuffix } {
  const now = Date.now();
...
export class TimeagoVVFFormatter extends TimeagoFormatter {
  ...
  private parse(value: number, unit: CustomUnit, suffix: CustomSuffix, now: number, then: number) {
